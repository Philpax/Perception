...
bool InitConfig()
{
	...
	HRESULT hr = RegCreateKeyEx(HKEY_CURRENT_USER, sk, 0, NULL, 0, 0, NULL, &hKey, NULL);

	if (hr != ERROR_SUCCESS)
	{
		FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, NULL, errorBuffer, 512, NULL );

		LogF( "[Perception] Error creating registry key: %s", errorBuffer );
	}

	hr = RegOpenKeyEx(HKEY_CURRENT_USER, sk, 0, KEY_ALL_ACCESS , &hKey);

	if (hr != ERROR_SUCCESS)
	{
		FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, NULL, errorBuffer, 512, NULL );

		LogF( "[Perception] Error opening registry key: %s", errorBuffer );
	}
	...
	if (hr != ERROR_SUCCESS)
	{
		...
		LogF( "[Perception] Error writing to registry: %s", errorBuffer );
	}
	...
	if (hr != ERROR_SUCCESS)
	{
		...
		LogF( "[Perception] Error closing registry key: %s", errorBuffer );
	}
