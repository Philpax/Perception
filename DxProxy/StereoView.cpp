...
{
	Log( "[StereoView] Destroyed" );
}

void StereoView::Init(IDirect3DDevice9* dev)
{
	Log( "[StereoView] Initialized" );
...
void StereoView::InitTextureBuffers()
{
	...
	LogF( "[StereoView] Viewport width: %d | Backbuffer width: %d",
			viewport.Width,
			pDesc.Width );
...
void StereoView::InitVertexBuffers()
{
	...
	Log( "[StereoView] Initialized vertex buffers" );
}
...
void StereoView::SaveScreen()
{
	...
	LogF( "[StereoView] Saved screenshot: %s", fileName );
